...
/// YOUR CODE HERE
	...
	protected int sumNodes(IntTreeNode numRoot) {

		if (numRoot == null) {
			return 0;
		} else {
			return (numRoot.data + sumNodes(numRoot.left) + sumNodes(numRoot.right));
		}

	}

	public int countLeftNodes() {
		// add code her to count how many left children are in the tree. A left child is
		// a node
		// that appears as the root of the left-hand subtree of another node. An empty
		// tree has 0 left nodes.
		// you may use additional methods
		return countLeftNodes(overallRoot);
	}

	protected int countLeftNodes(IntTreeNode node) {
		int count = 0;
		if (node.left != null) {
